...
URL="${URL:-https://www.benekiva.com/}"
...
run_vnc_server() {
    ...
    if [ -e "${VNC_SERVER_PASSWORD}" ]
    then
        export X11VNC_AUTH="-rfbauth ${VNC_SERVER_PASSWORD}"
        log_i "The VNC server will ask for a password"
    else
        log_w "The VNC server will NOT ask for a password"
    fi

    x11vnc -ncache 10 -display ${DISPLAY} -wait 20 -forever ${X11VNC_AUTH} &
    # wait $!
}
